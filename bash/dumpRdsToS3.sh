function dump_rds_to_s3() {
  function output_dump_rds_to_s3() {
    ...
    required options:
      -l --s3_bucket_location     an s3 location (bucket plus path, not a file/object). e.g.: "s3://my-bucket/my-suffix-path/"
...
END
  ...
  local db_type db_user db_password s3_location secret_json
  ...
  s3_bucket_location="$(echo "${s3_bucket_location}" | sed -E 's:/+$::')"
