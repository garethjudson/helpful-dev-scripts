...
As this really is a repository to store things that I use (or have used) so I don't lose this knowledge.  
...
The 'js' scripts depend on Node.js being installed. Tested with version 16. 
...
## run
A script that generalizes the running of projects. 
...
- `gradle run` command (preference use of ./gradlew if available)
- `mvn mn:run` command (preference use of ./mvnw if available)
if there is a pom.xml or build.gradle then, fall back to assuming it's a SpringBoot project:
- `gradle bootrun` command (preference use of ./gradlew if available)
- `mvn spring-boot:run` command (preference use of ./mvnw if available)
if there is a package.json then assume npm:
- `npm start`
if there's a docker-compose.yml:
- `docker-compose up`


## build
Supports gradle and maven, preference for wrappers (e.g. gradlew or mvnw). Forwards arguments to function calls.
...
## test
Supports npm, gradle and maven, preference for wrappers (e.g. gradlew or mvnw). Forwards arguments to function calls.
...
## xmlToJson
Convert a xml document to json. This uses yq to do the conversion. It's really just a shortcut syntax.
```
...
The function expects input from stdin so use standard piping to call the function:
```
...
Undercover this uses 'diff' for the output, so it can be interpreted the same way. 
...
## diffSync
Utility that will sync directories, from or to s3, or locally. The build in aws sync relies on filesize and modified date to determine when to sync files. This is for when you actually need to know if the file has changed. THIS UTILITY WILL DELETE FILES FROM THE DESTINATION 'to' DIRECTORY/BUCKET. Use at your own risk.


Sometimes sending events into s3 because you have overwritten a file that hasn't changed is not wanted. Sometimes filesize does not change but the file has changed. In some circumstances the last modified dates will not be the same, when the files have not changed. The solution is to sync the bucket to an empty temporary directory and then compare this to the local directory. This is not intended for buckets that are huge, use at your own risk of waiting forever! 
...
You can then pipe that output to disk, the cloud or wherever you feel like.
...
## restore_db
restore_db is a function that wraps database restore commands and restores a gzipped dump to the appropriate database.

It reads the dump from stdin, so you can supply it from the command line from wherever you choose. 
